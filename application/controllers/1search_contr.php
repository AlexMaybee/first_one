...
class Search_contr extends CI_Controller{
    ...
    public function index() //все перенес в модель
    {
        ...
        if($loginUserParameter)
        {
            ...
            if($this->users->countMatchesOfSearch() === false or $this->users->globalUsers() == false)
            {
                $this->session->set_flashdata('error', 'There is no matches or wrong data');
            ...
            else
            {
                ...
                $data['count'] = $this->users->countMatchesOfSearch();
