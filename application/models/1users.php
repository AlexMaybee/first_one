...
class Users extends CI_Model{
  ...
  /*  public function globalUsers($num,$offset) //пример правильного запроса с поиском по всем полям
    ...
    public function globalUsers() //пример правильного запроса с поиском по всем полям
    {
        ...
        {
            if($this->countMatchesOfSearch() === false)
            {
                return false;
            }
            else
            {
                ...
                $config=array(
                    'base_url' => base_url().'/search_contr/index/',//'/users/globalUsers', //метод должен быть ОБЯЗАТЕЛЬНО!
                    'total_rows' => $this->countMatchesOfSearch(),// $this->photos_model->countFotos(), //Сколько строк массива загружается из базы
